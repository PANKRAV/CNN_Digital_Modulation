...
parfor m_idx = 1:num_mods
    ...
    for i = 1:num_images
        for snr_id = 1:3
            if snr_id == 1
                snr = unifrnd(5, 10);
                snr_label = sprintf('Low%.2f', snr);
            elseif snr_id == 2
                snr = unifrnd(10, 20);
                snr_label = sprintf('Medium%.2f', snr);
            else
                snr = unifrnd(20, 30);
                snr_label = sprintf('High%.2f', snr)'; 
            end
            for pn_id = 1:2
                if pn_id == 1
                    pn = unifrnd(0, 0.08);
                ...
                for iq_id = 1:2
                    ...
                    for jam_id = 1:2
                        ...
                        csv_lines(idx) = sprintf( '%d,%s,%.3f,%.3f,%.3f,%.3f',i, mod_label, pn, iq, jam, snr);
